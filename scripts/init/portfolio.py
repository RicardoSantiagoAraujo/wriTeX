...
def main():
    ...
    print_progress_msg(f"Removed old build folder {bold+green}{build_folder}{reset} from new portfolio")

    # Remove template markdown auxiliary files
    delete_md_aux(new_folder)
    print_progress_msg(f"Removed old {bold+green}_markdown{reset} folder from new portfolio ")
