...
class MyHandler(FileSystemEventHandler):
    ...
    def on_generic(self, event, task: str) -> None:
        ...
        if (
            ...
            and not any(
                p in file_path for p in self.paths_to_ignore
            )  # check if file is excluded or part of an excluded folder
        ):
...
def watcher(
    function_to_trigger: Callable,
    function_arguments: argparse.Namespace,
    path_to_watch: str,
    ...
    observer = Observer()
    observer.schedule(event_handler, path=path_to_watch, recursive=True)  # to watch subfolders
    observer.start()
